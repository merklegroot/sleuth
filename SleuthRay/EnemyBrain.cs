...
internal interface IEnemyBrain
{
    ...
    bool Tick(
        ...
        List<(Vector2 Pos, Vector2 Vel, bool FromPlayer, float HitCooldown, string Name, int CatVariant)> bullets,
        ...
        if (wantsToShoot && e.ShootCooldown <= 0f && losToAimTarget && inShootBand)
        {
            ...
            if (e.BurstShotsLeft > 0 && e.BurstShotTimer <= 0f)
            {
                ...
                if (aimDir.LengthSquared() > 1e-6f)
                {
                    ...
                    e.FaceDir = aimDir;
                    bullets.Add((e.WorldPos + aimDir * p.BulletSpawnPad, aimDir * p.BulletSpeed, false, 0f, "", -1));
                    fired = true;
                }
